...
class ClassWeatherSearch extends Component {
  constructor() { 
    super();
    this.state = { text: 'not working', city: 'testcity', state: 'testState', showWeather: false}; 
  ...
  handleSubmit = (e) => {
    ...
    alert(
        ...
        //@todo, search users
        this.setState({text: '', city: validation[1], state: validation[2], showWeather: true})
  ...
  render() { 
    ...
    <div className='py-8'>
        {this.state.showWeather ? <WeatherResults city={this.state.city} state={this.state.state}/> : <p>Please Enter a Location</p> }
      </div>
    </div>
